# 🏢 [1단계] Brand Identity & Campaign Definition: OVERTIME

**"대한민국에서 살아남는 K-직장인의 우정"**

본 문서는 AI 기반 브랜드 광고 패키지 제작을 위한 기획 의도와 브랜드 아이덴티티를 정의한 문서입니다.
...
- **물리적 각성을 넘어선 정서적 연대감:** 타사 에너지 음료가 '개인의 강박적 생산성'을 강조할 때, 오버타임은 지친 밤을 함께 버텨주는 동료의 따뜻한 위로와 피로회복을 전면에 내세움.

## 3. 캠페인 및 메시지 전략 (Campaign Strategy)

- **캠페인 목표:** 제품의 시각적 각인(Visual Imprinting) 및 브랜드 경험을 통한 감성적 유대감 형성
- **핵심 메시지 (One-Line Message):**
  > "대한민국에서 성실한 사람으로 산다는 것, 그것은 우리에겐 회복하는 힘이 있다는 것."
...
- **오디오/BGM:** Gemini 3.1 flash TTS, Gemini 3.1 Pro (AiStudio) 
- **통합 편집:** Canva (컷 편집, 사운드 배열)

⚠️ **리스크 대응 전략**

- **일관성 유지:** 씬 이미지를 프롬프트로 변환하여 소스 일관성을 유지하고, 캐릭터 및 화풍의 불일치를 방지하기 위해 스타일 레퍼런스를 고정함.
- **물리 법칙 왜곡 방지:** 몽타주 기법(7개 컷 분할)을 도입하여 Sora 2 및 Gemini의 4초~10초 생성 최적화 범위를 준수하고 물리적 왜곡을 최소화함.
- **도구 접근성 대응:** 크레딧/플랜 제한을 고려하여 Kling, google flow, gemini Pro, Aistudio 등 대체 도구 리스트를 확보함.

## 5. 스토리보드 구성 요약 (34초 광고)
...
| **2-1**  | 4s     | 호의의 시작     | 트래킹 / 책상 위를 부드럽게 미끄러지는 메탈릭 캔, 표면에 맺힌 차가운 물방울(결로) 디테일    | "당신의 야근 회복제는..."                  |
...
| **4-1**  | 8s     | 해방/각인      | 하이앵글 / 퇴근길 줌아웃**(마지막 3~5초 구간 'OVERTIME' 로고 애니메이션 팝업)** | "풀려라 야근! 풀려라 피로! / 우리의 퇴근은 오버타임!" |


## 6. 프롬프트 개선 사례 (Cut 3-1)

*스토리보드 세부 프롬프트 작성 후 업데이트 예정*
